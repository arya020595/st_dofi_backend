...
- **Controllers** (`app/controllers`) — parse params, call one policy/service/query, render a response. No business rules, no direct multi-step ActiveRecord orchestration.
...
- **Services** (`app/services`, create as needed) — business logic and multi-step workflows. Use `dry-monads` `Success`/`Failure` results instead of raising for expected failure paths; controllers pattern-match on the result instead of branching on exceptions. Delegate SQL/ActiveRecord query building to a `Query` object (`app/queries`) rather than inlining multi-line `joins`/`group`/`pluck` chains in the service itself.
- **Queries** (`app/queries`, create as needed) — read-only SQL/ActiveRecord query building only: joins, scopes, `group`/`pluck`/aggregation. No business rules, no response formatting, no side effects. Namespace/module mirrors the caller, suffixed `Query` (e.g. `Fisherman::Dashboard::TopFishesQuery`). A `*Query` class always exposes `self.call(...)`. For a pure read with no logic beyond fetching data, a Controller may call the Query directly — don't create a Service whose only job is forwarding the call to one Query. Once real business logic sits on top of the query result (a computed metric, a guard, a side effect), wrap it in a Service and have the Controller call that Service instead. Use a Query object once a lookup is complex enough to be worth naming/reusing; a simple one-line model scope can still live inline in a service.
