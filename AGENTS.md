...
> **This file is a full, synced mirror of [`CLAUDE.md`](CLAUDE.md).** Different agents look for
> different filenames by default — Claude Code reads `CLAUDE.md`, Codex and most other tools read
> `AGENTS.md` — so rather than trust every agent to notice and follow a "see CLAUDE.md" pointer,
> the actual rules live in both files. **`CLAUDE.md` is the authoritative copy**; if the two ever
> disagree, `CLAUDE.md` wins and this file is stale. **Whoever edits one must edit the other in the
> same change.** [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) and
> [`docs/architecture/thin-controllers-and-query-objects.md`](docs/architecture/thin-controllers-and-query-objects.md)
> are the deeper worked examples behind the contracts below — read them before writing or modifying
> code in an area they cover, not just this summary.
