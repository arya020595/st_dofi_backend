    ...
    module Approvals
      class DocumentsControllerTest < ActionDispatch::IntegrationTest
        ...
        test "index requires the list/view permission" do
          get "/api/v1/approvals/documents", headers: @plain_headers

          assert_response :forbidden

          get "/api/v1/approvals/documents", headers: @admin_headers

          assert_response :ok
        end

        test "approve transitions the document from pending to approved" do
          post "/api/v1/approvals/documents/#{@document.id}/approve", headers: @admin_headers
        ...
        test "approve without permission is forbidden" do
          post "/api/v1/approvals/documents/#{@document.id}/approve", headers: @plain_headers

          assert_response :forbidden
        end

        test "request_amendment records the remarks and moves the document to amendment_required" do
          post "/api/v1/approvals/documents/#{@document.id}/request_amendment",
