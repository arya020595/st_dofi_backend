    ...
    module Approvals
      class VesselsControllerTest < ActionDispatch::IntegrationTest
        ...
        test "index requires the list/view permission" do
          get "/api/v1/approvals/vessels", headers: @plain_headers

          assert_response :forbidden

          get "/api/v1/approvals/vessels", headers: @admin_headers

          assert_response :ok
        end

        test "approve transitions the vessel from pending to approved" do
          post "/api/v1/approvals/vessels/#{@vessel.id}/approve", headers: @admin_headers
        ...
        test "approve without permission is forbidden" do
          post "/api/v1/approvals/vessels/#{@vessel.id}/approve", headers: @plain_headers

          assert_response :forbidden
        end

        test "request_amendment records the remarks and moves the vessel to amendment_required" do
          post "/api/v1/approvals/vessels/#{@vessel.id}/request_amendment",
