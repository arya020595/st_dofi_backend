    ...
    module CompanyProfiles
      class FishingGearsControllerTest < ActionDispatch::IntegrationTest
        ...
        test "index requires the list/view permission" do
          get "/api/v1/company_profiles/#{@company_profile.id}/fishing_gears", headers: @plain_headers

          assert_response :forbidden

          get "/api/v1/company_profiles/#{@company_profile.id}/fishing_gears", headers: @admin_headers

          assert_response :ok
        end

        test "create links the company to a master fishing gear, pending approval" do
          ...
          assert_difference("CompaniesFishingGear.count", 1) do
            post "/api/v1/company_profiles/#{@company_profile.id}/fishing_gears", params: params,
          ...
          assert_equal ["pending", @fishing_gear.id], [data["approval_status"], data["fishing_gear_id"]]
        end

        test "destroy soft-deletes the company's fishing gear link" do
          gear = create(:companies_fishing_gear, company_profile: @company_profile)

          delete "/api/v1/company_profiles/#{@company_profile.id}/fishing_gears/#{gear.id}", headers: @admin_headers
