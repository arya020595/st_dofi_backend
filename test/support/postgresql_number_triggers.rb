module PostgreSqlNumberTriggers
  LOCK_ID = 20_260_921

  CAPTURE_REPORT_SQL = <<~SQL.squish.freeze
  ...
  COMPANY_PROFILE_SQL = <<~SQL.squish.freeze
  ...
  def self.install!
    connection.transaction do
      ...
      connection.execute(CAPTURE_REPORT_SQL)
      connection.execute(COMPANY_PROFILE_SQL)
    end
  end
